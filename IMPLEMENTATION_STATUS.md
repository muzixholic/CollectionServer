...
| 4. External Providers | 도서/음악/영화 Provider | ⚡ 대부분 완료 | Google·Kakao·Aladin·MusicBrainz·Discogs·UpcItemDb+TMDb 운영 준비 / TMDb·OMDb Stub 유지 |
...
- **외부 Provider 커버리지**: 도서/음악은 운영 수준, 영화는 UpcItemDb 브리지로 UPC 대응. (TMDb/OMDb Stub 유지)
...
## 남은 과제
1. **영화 UPC 직접 지원** – UPC→IMDb/TMDb 상용 데이터 또는 제목/ID 변환 서비스를 조사해 TMDb/OMDb Provider를 완전 구현합니다.
...
## 다음 단계
- UPC→IMDb 매핑 조사로 TMDb/OMDb Provider를 고도화합니다.
